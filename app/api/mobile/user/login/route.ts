...
import {Admin, ConnectedAdmin, ConnectedMember, ConnectedUser, Member} from '@/types'
...
export async function POST(request: NextRequest) {
    ...
    console.log('api/mobile/test/user/login ~> Tentative de connection en cours :', userMail);
    try {
        await databaseConnecter();
        if (!userMail || !userPassword) {
            console.log('api/mobile/test/user/login ~> Erreur de mail')
        ...
        if (userType === 'member') {
            ...
            console.log('api/mobile/test/user/login ~> utilisateur connecté :', userMail)
        ...
        console.log('api/mobile/test/user/login ~> Erreur de type d\'utilisateur')
        return applyCorsForMobiles(request, 'type d\'utilisateur incorrect', 400)
  } catch (error) {
    console.log('api/mobile/test/user/login ~> error :', error)
